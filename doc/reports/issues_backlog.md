...
- 標準的な模型眼を1つ選び、既存の`refractive`面・`sensor`（網膜相当）の組み合わせとしてプリセット化できるか検討する。
- `system_type: afocal`の終端`eye_reference`後段へ別の光学系を接続する構成を検討する。現行の`focal`/`afocal`二択ではafocal→focal接続を表せないため、エンジン仕様拡張の要否を判断する。
- 眼球モデルの瞳孔径、屈折異常、調節状態のパラメータ化範囲を決める。初期版は固定の標準模型眼だけでよい。
- 既存のafocal評価値（射出瞳、角度MTF等）との関係を整理し、責務の重複を避ける。
