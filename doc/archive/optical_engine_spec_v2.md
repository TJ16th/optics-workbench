...
surfaces:
  - id: M1            # 主鏡（凹）
    kind: mirror
    radius_mm: -2000.0
    thickness_after_mm: -700.0     # 反射後、-X方向へ700mm
    semi_diameter_mm: 100.0
  - id: M2            # 副鏡（凸）
    kind: mirror
    radius_mm: -500.0
    thickness_after_mm: 800.0      # 再反射後、+X方向へ800mm
...
総合評価値は、各評価値の正規化値の重み付き和として定義する。

```
merit =
  w1 * normalized_rms_spot
+ w2 * normalized_distortion
+ w3 * normalized_lateral_color
+ w4 * normalized_field_curvature
+ w5 * normalized_astigmatism
+ w6 * normalized_relative_illumination_loss
+ w7 * normalized_mtf_loss
...
実現不能な候補（10.3節の違反）には status: infeasible と大きな constraint_penalty を返し、外部最適化が探索空間から除外できるようにする。

### 25.4 将来の勾配提供【v2新設】

将来拡張として、微分可能トレースカーネル（27.5節のJAX方針）により merit の設計変数に対する勾配 ∂merit/∂p をAPIで返すことを視野に入れる。これにより外部最適化は有限差分（変数数×2回の評価）を省け、評価回数を桁で削減できる。初期版のAPIスキーマは、応答に gradients フィールドを追加できる構造（後方互換）としておく。
...
```json
{
  ...
  "evaluation": {
    "fields": [
      {"theta_y_deg": 0, "theta_z_deg": 0, "weight": 1.0},
      {"theta_y_deg": 0, "theta_z_deg": 10, "weight": 0.8},
      {"theta_y_deg": 0, "theta_z_deg": 20, "weight": 0.6}
    ],
    "wavelengths": [
      {"wavelength_nm": 486.13, "weight": 0.25},
      {"wavelength_nm": 587.56, "weight": 0.50},
      {"wavelength_nm": 656.27, "weight": 0.25}
    ],
    "metrics": [
      "rms_spot_radius",
      "distortion",
