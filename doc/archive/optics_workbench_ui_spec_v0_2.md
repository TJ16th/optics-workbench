# Optics Workbench UI 要求仕様・技術仕様書 v0.2
...
```json
{
  ...
  "message": "S4-S5間の空気間隔が負になっています。",
  "surface_ids": ["S4", "S5"],
  "group_ids": ["G2"],
  "suggested_action": "群位置またはthickness_after_mmを見直してください。",
  "details": {
    "gap_mm": -0.35
  }
}
...
## 27.2 Explanation Panel

教育モードまたはPreviewタブでは、解析結果に対応する簡易説明を表示できる。

例：

```text
絞りを小さくすると、通過する光線束が細くなり、球面収差の影響が小さく見えます。
ただし、回折は初期版の幾何PSFには含まれていません。
```
